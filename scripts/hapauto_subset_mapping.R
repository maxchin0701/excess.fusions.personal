...
mat <- as.matrix(read.csv("../data/transition_matrix/transition_matrix_hapauto.csv",
                          as.is=T,header = T))
Qmat <- as.matrix(read.csv("../data/transition_matrix/Q_matrix_subset_hapauto.csv",
                           as.is=T,header=T))
...
cols <- c(viridis(49))
names(cols) <- c(1:49)
plotSimmap(hists.fixed[[1]],col=cols,fsize = 0.05,lwd=1,pts = T)
hists.fixed.alt <- hists.fixed
