...
for(j in 1:15){
  j=13
  #### READ TREE AND QMAT ####
  tree <- read.tree(paste0("../data/trees/deprecated/subtrees/tree_",j,".nwk"))
  
  Qmat <- as.matrix(read.csv(paste0("../data/transition_matrix/deprecated/split_matrices/split_matrix_",j,".csv"),
                             as.is=T,header=T))
  ...
  for(i in 1:nrow(data.subset)){
    if(is.na(data.subset$hapauto[i]) == T ||
       data.subset$codedSCS[i] == "unclear"){
      data.subset$sim.state[i] <- NA
    } else if(data.subset$codedSCS[i] == "unfused"){
      data.subset$sim.state[i] <- data.subset$hapauto[i] - chrmrng[1] + 1
    } else if(data.subset$codedSCS[i] == "fused"){
      data.subset$sim.state[i] <- (data.subset$hapauto[i] - chrmrng[1] + 1) +
  ...
  for(i in 1:nrow(pruned.data)){
    hit <- which(data.subset$tree.name == row.names(pruned.data)[i])
  ...
  rm(chrmrng,hit,i,simcoding,weights)
  ...
  #Establish palette
  col <- viridis_pal(direction=-1,option="D")(ncol(pruned.data))
  cols <- setNames(c(col,"red"),c(1:ncol(pruned.data),"fail"))
  
  #Plot each of 100 trees
  for(i in 1:100){
    
    cairo_pdf(paste0("../figures/subtree_simmap_plots/subtree_",j,"/tree_",i,".pdf"))
    plotSimmap(sim.out[[i]],
               cols,
               fsize=sqrt(15/length(sim.out[[i]]$tip.label)),
               lwd = 1)
    dev.off()
  ...
  for(i in 1:100){
    ...
    #Save object
    write.csv(times, paste0("../data/subtree_simmap_out/subtree_",j,"/dwelling_times/dwelling_times_",i,".csv"),quote=F,row.names=T)
  }
  
  #Save transitions
  write.csv(count,paste0("../data/subtree_simmap_out/subtree_",j,"/transitions.csv"),quote=F,row.names=T)

}

write.simmap(result,file="test.nex",version = 1.5,format="nexus")
x <- read.simmap(file="test.nex",version = 1.5)
